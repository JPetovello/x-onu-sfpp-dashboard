...
Email notifications are delivered through Gmail SMTP using implicit TLS on port `465`.

To enable Email notifications:

1. Enable 2-Step Verification for the Google account if it is not already enabled.
2. Create a Google App Password for the dashboard.
3. Open the **Alerts & Notifications** page.
4. Enter the Google account email, App Password, and one recipient email address.
5. Enable Email notifications.
6. Save the settings.

Use a Google App Password only. Do not enter the normal Google account password. Google Workspace addresses using a custom domain are supported when the account can authenticate through Gmail SMTP.

The App Password is stored in the persistent notification configuration. Treat the dashboard's persistent `/data` directory and its backups as sensitive.
...
Discord webhook URLs, Gotify application tokens, Pushover credentials, ntfy access tokens, private Generic Webhook URLs, and Gmail App Passwords should be treated as sensitive configuration.
...
Alert and notification configuration is stored in the dashboard's persistent SQLite database. This includes a Gmail App Password when Email notifications are configured. Treat the persistent `/data` directory as sensitive if notification credentials or private notification endpoint URLs have been configured.
